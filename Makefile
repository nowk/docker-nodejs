...
# node-shared-volume creates a shared volume on the global node_modules
# NOTE the shared volume path may vary with each version
node-shared-volume:
	docker run -v /usr/local/lib/node_modules --entrypoint node --name $(NAME)v$(VERSION) $(NAME):$(VERSION) --version
